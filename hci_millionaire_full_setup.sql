-- =============================================================================
-- HCI MILLIONAIRE — COMPLETE DATABASE SETUP
-- Run this entire script in pgAdmin to create the full database with user,
-- all tables, functions, triggers, stored procedures, seed data, and permissions.
-- =============================================================================

-- 1. CREATE DB USER (run separately in pgAdmin's default 'postgres' database)
-- Connect to 'postgres' database first, then run these lines:
-- CREATE ROLE hci_user WITH LOGIN PASSWORD 'hci_pass';
-- CREATE DATABASE hci_millionaire OWNER hci_user;
-- Then connect to 'hci_millionaire' database and run everything below.

-- 2. SCHEMA — all 12 tables
...
CREATE TABLE IF NOT EXISTS "Player" (
  ...
  CONSTRAINT fk_player_phone_hint
    FOREIGN KEY ("lifeLinePhoneResult") REFERENCES "PhoneAPeerHint" ("PhoneAPeerHintId")
    ON DELETE SET NULL DEFERRABLE INITIALLY IMMEDIATE,
  CONSTRAINT fk_player_note_hint
    FOREIGN KEY ("lifeLineNotesResult") REFERENCES "CourseNoteHint" ("CourseNoteHintId")
    ON DELETE SET NULL DEFERRABLE INITIALLY IMMEDIATE
);
...
-- Returns per-game result with safety net and final prize calculation
...
BEGIN
    ...
    WITH player_answers AS (
        SELECT pga."QuestionId", pga."isCorrect", q."PrizeLevelId",
               pl."prizeValue", pl."isSafetyNet"
    ...
    SELECT earned, safety_net, correct_count, total_count, GREATEST(earned, safety_net)
    FROM aggregates;
END;
$$;

-- Returns player-level career stats for leaderboard display
CREATE OR REPLACE FUNCTION fn_GetPlayerProfile(p_player_id INTEGER)
RETURNS TABLE(
    "PlayerId" INTEGER,
    "username" VARCHAR,
    "totalGames" BIGINT,
...
BEGIN
    ...
    SELECT
        p."PlayerId",
        p.username::VARCHAR,
        COALESCE(lb."totalGames", 0)::BIGINT,
        COALESCE(lb."totalCorrect", 0)::BIGINT,
        COALESCE(lb."totalWins", 0)::BIGINT
    FROM "Player" p
    LEFT JOIN "Leaderboard" lb ON lb."PlayerId" = p."PlayerId"
...
-- Creates a new player with auth fields and initialises leaderboard entry
...
-- Creates a new game with 1–4 players, optional categories, and generates GAM### code
...
BEGIN
    ...
    END IF;
    IF p_player2 IS NOT NULL AND NOT EXISTS (SELECT 1 FROM "Player" WHERE "PlayerId" = p_player2) THEN
        RAISE EXCEPTION 'Player 2 with ID % does not exist', p_player2;
    END IF;
    IF p_player3 IS NOT NULL AND NOT EXISTS (SELECT 1 FROM "Player" WHERE "PlayerId" = p_player3) THEN
        RAISE EXCEPTION 'Player 3 with ID % does not exist', p_player3;
    END IF;
    IF p_player4 IS NOT NULL AND NOT EXISTS (SELECT 1 FROM "Player" WHERE "PlayerId" = p_player4) THEN
...
-- Retrieves up to 15 active questions for a game, ordered by prize level
...
BEGIN
    OPEN p_questions FOR
        SELECT q."QuestionId", q."questionCode", q."question",
               c."name" AS category, pl."PrizeLevelId", pl."prizeLevelCode",
               pl."prizeValue", pl."isSafetyNet"
        FROM "Question" q
...
-- Records a player's answer, handles elimination, and auto-ends game when one player remains
...
BEGIN
    ...
    INSERT INTO "PlayerGameAnswer" (
        "playerGameAnswerCode", "GameId", "PlayerId", "QuestionId",
        "AnswerId", "isCorrect", "questionSequence"
    ) VALUES (
        v_answer_code, p_GameId, p_PlayerId, p_QuestionId,
        p_AnswerId, p_isCorrect, p_questionSequence
    );

    -- Elimination handling
    p_playerEliminated := NOT p_isCorrect;

    IF p_playerEliminated THEN
        -- Override prize value with highest safety net reached
        SELECT COALESCE(MAX(pl2."prizeValue"), 0)
        INTO v_safety_net_value
        FROM "PlayerGameAnswer" pga2
        JOIN "Question" q2 ON q2."QuestionId" = pga2."QuestionId"
        JOIN "PrizeLevel" pl2 ON pl2."PrizeLevelId" = q2."PrizeLevelId"
        WHERE pga2."GameId" = p_GameId
          AND pga2."PlayerId" = p_PlayerId
          AND pga2."isCorrect" = true
          AND pl2."isSafetyNet" = true;

        p_prizeValue := v_safety_net_value;

        -- Count remaining active (correct-answering) players in this game
        SELECT COUNT(DISTINCT pga3."PlayerId")
        INTO v_active_players
        FROM "PlayerGameAnswer" pga3
        WHERE pga3."GameId" = p_GameId
          AND pga3."PlayerId" != p_PlayerId
          AND pga3."isCorrect" = true;
...
-- Handles all four lifeline types: 5050, Phone, Notes, AskClass
...
BEGIN
    IF p_lifelineType = 'AskClass' THEN
        IF NOT EXISTS (SELECT 1 FROM "Player" WHERE "PlayerId" = p_PlayerId AND "lifeLineAskClass" = true) THEN
        ...
        UPDATE "Player" SET "lifeLineAskClass" = false, "lifeLineAskClassResult" = p_result
        WHERE "PlayerId" = p_PlayerId;

    ELSIF p_lifelineType = '5050' THEN
        IF NOT EXISTS (SELECT 1 FROM "Player" WHERE "PlayerId" = p_PlayerId AND "lifeLine5050" = true) THEN
            RAISE EXCEPTION '5050 lifeline has already been used by player %', p_PlayerId;
        END IF;
        SELECT "AnswerId" INTO v_wrong_answer1
        FROM "Answer" WHERE "QuestionId" = p_QuestionId AND NOT "isCorrect"
        ORDER BY random() LIMIT 1;
        SELECT "AnswerId" INTO v_wrong_answer2
        FROM "Answer" WHERE "QuestionId" = p_QuestionId AND NOT "isCorrect"
          AND "AnswerId" <> v_wrong_answer1 ORDER BY random() LIMIT 1;
    ...
    ELSIF p_lifelineType = 'Phone' THEN
        IF NOT EXISTS (SELECT 1 FROM "Player" WHERE "PlayerId" = p_PlayerId AND "lifeLinePhone" = true) THEN
            RAISE EXCEPTION 'Phone lifeline has already been used by player %', p_PlayerId;
        END IF;
        SELECT "PhoneAPeerHintId", "avatarName", "hintText"
        INTO v_hint_id, v_avatar_name, v_hint_text
        FROM "PhoneAPeerHint"
        WHERE "QuestionId" = p_QuestionId AND "isActive" = true
        ORDER BY random() LIMIT 1;
        IF v_hint_id IS NULL THEN
            RAISE EXCEPTION 'No active Phone a Peer hints available for question %', p_QuestionId;
        END IF;
        p_result := json_build_object('avatarName', v_avatar_name, 'hint', v_hint_text)::TEXT;
        UPDATE "Player" SET "lifeLinePhone" = false, "lifeLinePhoneResult" = v_hint_id
        WHERE "PlayerId" = p_PlayerId;

    ELSIF p_lifelineType = 'Notes' THEN
        IF NOT EXISTS (SELECT 1 FROM "Player" WHERE "PlayerId" = p_PlayerId AND "lifeLineNotes" = true) THEN
            RAISE EXCEPTION 'Notes lifeline has already been used by player %', p_PlayerId;
        END IF;
        SELECT "CourseNoteHintId", "noteText"
        INTO v_hint_id, v_hint_text
        FROM "CourseNoteHint"
        WHERE "QuestionId" = p_QuestionId AND "isActive" = true
        ORDER BY random() LIMIT 1;
        IF v_hint_id IS NULL THEN
            RAISE EXCEPTION 'No active Course Note hints available for question %', p_QuestionId;
        END IF;
        p_result := json_build_object('note', v_hint_text)::TEXT;
        UPDATE "Player" SET "lifeLineNotes" = false, "lifeLineNotesResult" = v_hint_id
        WHERE "PlayerId" = p_PlayerId;

    ELSE
        RAISE EXCEPTION 'Invalid lifeline type: %. Must be AskClass, 5050, Phone, or Notes', p_lifelineType;
    END IF;
END;
$$;

-- Ends a game session, determines winner by score, and recalculates leaderboard
...
BEGIN
    ...
    UPDATE "Game"
    SET "status" = p_status, "winner" = p_winnerId, "endTime" = now()
...
-- Recalculates rank for all leaderboard entries by best score
...
-- 7. RESET SEQUENCES (seed data uses explicit IDs)
SELECT setval('"Category_CategoryId_seq"', COALESCE(MAX("CategoryId"), 1)) FROM "Category";
SELECT setval('"PrizeLevel_PrizeLevelId_seq"', COALESCE(MAX("PrizeLevelId"), 1)) FROM "PrizeLevel";
SELECT setval('"Question_QuestionId_seq"', COALESCE(MAX("QuestionId"), 1)) FROM "Question";
SELECT setval('"Answer_AnswerId_seq"', COALESCE(MAX("AnswerId"), 1)) FROM "Answer";
SELECT setval('"CourseNoteHint_CourseNoteHintId_seq"', COALESCE(MAX("CourseNoteHintId"), 1)) FROM "CourseNoteHint";
SELECT setval('"PhoneAPeerHint_PhoneAPeerHintId_seq"', COALESCE(MAX("PhoneAPeerHintId"), 1)) FROM "PhoneAPeerHint";
SELECT setval('"Player_PlayerId_seq"', COALESCE(MAX("PlayerId"), 1)) FROM "Player";
SELECT setval('"Game_GameId_seq"', COALESCE(MAX("GameId"), 1)) FROM "Game";
SELECT setval('"GameCategory_GameCategoryId_seq"', COALESCE(MAX("GameCategoryId"), 1)) FROM "GameCategory";
SELECT setval('"PlayerGameAnswer_PlayerGameAnswerId_seq"', COALESCE(MAX("PlayerGameAnswerId"), 1)) FROM "PlayerGameAnswer";
SELECT setval('"Leaderboard_LeaderboardId_seq"', COALESCE(MAX("LeaderboardId"), 1)) FROM "Leaderboard";

-- =============================================================================
-- 8. VERIFICATION — run this separately after setup to confirm everything exists
-- =============================================================================
-- Copy and paste the block below into a new pgAdmin query tab:

/*
-- Single-result summary
SELECT 'Tables' obj, (SELECT COUNT(*) FROM information_schema.tables
  WHERE table_schema='public' AND table_type='BASE TABLE') n UNION ALL
SELECT 'Sequences', (SELECT COUNT(*) FROM information_schema.sequences
  WHERE sequence_schema='public') UNION ALL
SELECT 'Functions', (SELECT COUNT(*) FROM information_schema.routines
  WHERE specific_schema='public' AND routine_type='FUNCTION'
    AND routine_name LIKE 'fn_%') UNION ALL
SELECT 'Triggers', (SELECT COUNT(*) FROM information_schema.triggers
  WHERE trigger_schema='public') UNION ALL
SELECT 'Procedures', (SELECT COUNT(*) FROM information_schema.routines
  WHERE specific_schema='public' AND routine_type='PROCEDURE'
    AND routine_name LIKE 'sp_%') UNION ALL
SELECT 'Indexes', (SELECT COUNT(*) FROM pg_indexes
  WHERE schemaname='public' AND indexname LIKE 'idx_%') UNION ALL
SELECT 'Categories (seed)', (SELECT COUNT(*) FROM "Category") UNION ALL
SELECT 'PrizeLevels (seed)', (SELECT COUNT(*) FROM "PrizeLevel") UNION ALL
SELECT 'Questions (seed)', (SELECT COUNT(*) FROM "Question") UNION ALL
SELECT 'Answers (seed)', (SELECT COUNT(*) FROM "Answer");

-- Detailed lists per object type
...
SELECT routine_name FROM information_schema.routines
  WHERE specific_schema='public' AND routine_type='FUNCTION'
    AND routine_name LIKE 'fn_%' ORDER BY routine_name;
...
SELECT routine_name FROM information_schema.routines
  WHERE specific_schema='public' AND routine_type='PROCEDURE'
    AND routine_name LIKE 'sp_%' ORDER BY routine_name;

SELECT '--- IDENTITY COLUMNS ---' AS info;
...
-- 9. GRANT PERMISSIONS
-- =============================================================================
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO hci_user;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO hci_user;
GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA public TO hci_user;
GRANT ALL PRIVILEGES ON SCHEMA public TO hci_user;
