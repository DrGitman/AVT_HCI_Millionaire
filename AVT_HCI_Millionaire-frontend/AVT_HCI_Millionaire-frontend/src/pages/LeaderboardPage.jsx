      ...
      .then((data) => {
        ...
        setScholars(
          data.entries.map((entry, index) => ({
            ...
            gamesPlayed: String(entry.totalGames ?? 0),
            accuracy: entry.totalGames ? `${Math.round((entry.totalCorrect / (entry.totalGames * 15)) * 100)}%` : '0%',
            learned: entry.totalCorrect ?? 0,
            avatar: (entry.name || entry.username || '??')
              .split(' ')
              .map((part) => part[0])
              .join('')
              .slice(0, 2)
